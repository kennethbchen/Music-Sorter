...
impl SupportedInput {
    fn from(path: PathBuf) -> Option<SupportedInput> {
        ...
        match path.extension() {
            Some(extension_str) => match extension_str.to_str().unwrap() {
                "zip" => {
...
impl SortableTag {
    ...
    fn path(&self) -> PathBuf {
        ...
        let root_folder = self
            .album_artist
            .as_ref()
            .unwrap_or_else(|| self.album.as_ref().unwrap());
...
fn parse_config() -> AppSettings {
    ...
    assert!(
        input_path
            .try_exists()
            .expect("Could not check if input folder exists."),
        "Input folder does not exist!"
    );
    assert!(
        output_path
            .try_exists()
            .expect("Could not check if output folder exists."),
        "Output folder does not exist!"
    );
    assert!(
        processed_input_path
            .try_exists()
            .expect("Could not check if processed input folder exists."),
        "Processed input folder does not exist!"
    );
...
fn get_destination_path(output_folder_path: &PathBuf, file: &SupportedInput) -> Option<PathBuf> {
    match file {
        ...
        SupportedInput::ZipInput(path) => {
            let Ok(zip) = File::open(path) else {
                return None;
            };
            ...
            for i in 0..zip.len() {
                // Check if this file is an mp3
                {
                    ...
                    let Some(filepath) = file.enclosed_name() else {
                        continue;
                    };

                    let Some(file) = SupportedInput::from(filepath) else {
                        continue;
                    };
                ...
                // Get song metadata
                let Ok(file) = &mut zip.by_index_seek(i) else {
                    continue;
                };
            ...
            let Some(sortable_tag) = sortable_tag else {
                return None;
            };
...
fn main() {
    ...
    for entry in fs::read_dir(&settings.input_path).unwrap() {
        ...
        match entry {
            Some(input) => {
                let Some(destination_path) = get_destination_path(&settings.output_path, &input)
                else {
                    ...
                    SupportedInput::ZipInput(input_path) => {
                        {
                            ...
                            let Ok(mut zip) = ZipArchive::new(zip) else {
                                println!(
                                    "Couldn't open {:?} as a zip file. Skipping...",
                                    &input_path
                                );
                                continue;
                            };
